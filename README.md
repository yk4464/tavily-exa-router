...
一个教 AI 编码助手**在一次性公网检索中选择 Tavily 或 Exa,并正确配置参数**的 agent skill。路由规则来自 2026 年 8 月的对标实测、官方参数审计、公开问题记录和带边界说明的社区评价,见 [`references/evidence.md`](references/evidence.md)。
...
| 学术内容 | 一般 | **强**(arxiv 密集) |
| 非英文内容(如中文)质量 | 参差 | **强** |
| 结果带发布日期 | 一般搜索实测 0/158 | **80/160 结果携带** |
| 结果带相关性评分 | **始终携带** | 不同模式不统一 |
| 免读链接直接出 LLM 答案 | **内置** | 经 `outputSchema` 实现 |
| 结构化 JSON 输出 | — | **内置** |
| 抓取已知 URL | **`/extract`，对高阻站点相对更稳** | **`/contents`，适合已索引页面** |
| 基础查询成本 | 约 $0.008 | 约 $0.007 |

本 skill 把这些编码为:10 秒路由表、模式选择规则(包括哪些快速模式虽然更快,却明显牺牲结果质量)、参数配方,以及坑清单(废弃参数、`category=company` + 日期过滤的 400 错误、内容计费和网页提示注入等)。
...
2. **模式规则** —— Tavily `basic` 为综合默认,`fast` 仅用于候选发现;Exa `instant`/`auto` 为常用入口,深度模式只在明确需要时使用
3. **范围边界** —— 一次性公网检索之外的活(监控、私有源、URL 安全、实时价格)明确拒绝并给出替代方向
4. **抓取指南** —— 已知 URL 用 Tavily `/extract`,包括哪些站点类型会拦它
5. **坑清单** —— 8 条实测或对照厂商文档验证的陷阱
...
└── references/
    ...
    ├── community-feedback.md  # 问题追踪、社区评价与独立对比
    └── provider-api-audit-2026-08-18.md  # 官方参数与实测边界
```
...
路由规则来自两家 API 的正面对比测试(质量、延迟、模式、功能、抓取、成本),以及 GitHub、HN、Reddit 和中文开发者论坛的公开反馈。最新测试日期:2026-08-18。
